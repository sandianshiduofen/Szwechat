...
;(function(exports){
  var KeyBoard = function(input, options){
    ...
    var height = options && options.height || '205px';
    ...
    cssStr += '#' + TABLE_ID + ' td{width:33%;border:1px solid #ddd;border-right:0;border-top:0;height:50px;}';
   /* if(!mobile){
      cssStr += '#' + TABLE_ID + ' td:hover{background-color:#52bb03;color:#FFF;}';
    }*/
    cssStr += '</style>';
     
    //Button
   /* var btnStr = '<div style="width:60px;height:28px;background-color:#52bb03;';
    ...
    var tableStr = '<table id="' + TABLE_ID + '" border="0" cellspacing="0" cellpadding="0">';
      ...
      tableStr += '<tr><td style="background-color:#D3D9DF;"><div style="height:100%;line-height:50px;font-size:16px;">完成</div></td><td>0</td>';
      tableStr += '<td style="background-color:#D3D9DF;"><<</td></tr>';
      tableStr += '</table>';
    ...
    function addEvent(e){
		...
		if (mobile) {
			clickEl.style.background='#aaa';
		};
	    if(clickEl.tagName.toLocaleLowerCase() === 'td' && value !== "<<"){
	        if(self.input){
		        if (self.input.value.length<6) {
    ...
    function addEventUp(e){
		var ev = e || window.event;
		var clickEl = ev.element || ev.target;
		clickEl.style.background='#fff';
		if(clickEl.tagName.toLocaleLowerCase() === 'td' && value !== "<<"){
	        clickEl.style.background='#D3D9DF';
		}else if(clickEl.tagName.toLocaleLowerCase() === 'div' && value === "完成"){
			clickEl.style.background='#D3D9DF';
		}
